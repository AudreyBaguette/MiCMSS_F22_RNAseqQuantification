...
tteTest <- topTags(eTest, n_genes, p.value = 0.05)
...
ttqlf <- topTags(qlf, n_genes, p.value = 0.05)
# likelihood ratio tests:
fit <- glmFit(DGE, design)
lrt <- glmLRT(fit)
ttlrt <- topTags(lrt, n_genes, p.value = 0.05)


save(tteTest, file = "../Data_solutions/tteTest.Rda")
#
